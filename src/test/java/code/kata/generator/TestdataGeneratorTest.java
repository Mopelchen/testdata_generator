...
public class TestdataGeneratorTest {
    ...
    public void test_generate_givenClassWithIntegerList_setFieldToListWithOneElementOfMAX_INT () throws Exception {
        ...
        assertThat(p.getStringList()).isNotNull().hasSize(1).contains("stringList");
        assertThat(p.getEnumFieldList()).isNotNull().hasSize(1).contains(EnumField.YEPP);
    }

    @Test
    public void test_generate_givenClassWithIntegerSet_setFieldToSetWithOneElementOfMAX_INT () throws Exception {

        SetClass p = underTest.generate(SetClass.class);

        assertThat(p).isNotNull();
        assertThat(p.getIntegerSet()).isNotNull().hasSize(1).contains(Integer.MAX_VALUE);
    }

    @Test
    public void test_generate_givenClassWithMap_thenPutOneKeyValuePair () throws Exception {

        MapClass p = underTest.generate(MapClass.class);

        assertThat(p).isNotNull();
        assertThat(p.getMapField()).isNotNull().hasSize(1).containsKey("mapField");
        assertThat(p.getMapField().get("mapField")).isNotNull().isEqualTo(Integer.MAX_VALUE);
    }
